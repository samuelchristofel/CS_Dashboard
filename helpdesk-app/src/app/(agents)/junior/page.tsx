...
export default function JuniorDashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const activeMyTickets = (myTicketsData.tickets || []).filter((t: Ticket) => t.status !== "CLOSED" && t.status !== "RESOLVED");
      ...
      const availableLowMedium = (availableData.tickets || []).filter((t: Ticket) => (t.priority === "LOW" || t.priority === "MEDIUM") && t.status !== "CLOSED" && t.status !== "RESOLVED");
  ...
  const canEscalate = selectedTicket && (selectedTicket.priority === "MEDIUM" || selectedTicket.priority === "HIGH") && selectedTicket.assigned_to?.id === userId;
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <button
                ...
                className={`text-lg font-bold transition-colors ${activeTab === "available" ? "text-slate-900" : "text-slate-400 hover:text-slate-600"}`}
              >
                Available to Claim
                {availableTickets.length > 0 && <span className="ml-2 text-sm bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full animate-pulse">{availableTickets.length}</span>}
              </button>
            </div>
