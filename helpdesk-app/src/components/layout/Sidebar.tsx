                ...
                <div className="flex items-center gap-3">
                    <div className={`size-10 rounded-full ${logoColor} flex items-center justify-center text-white shadow-lg`}>
                        <span className="material-symbols-outlined text-2xl">
                            {role === 'admin' ? 'admin_panel_settings' : role === 'it' ? 'build' : 'support_agent'}
                        </span>
                    </div>
                    <h1 className="text-slate-900 text-xl font-bold tracking-tight">
                        {role === 'admin' ? 'Admin Panel' : 'HelpDesk'}
                    </h1>
                </div>
            </div>
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <Link
                            key={item.href}
                            href={item.href}
                            className={`flex items-center gap-4 px-4 py-3 rounded-full transition-all ${active
                                ? 'bg-white text-slate-900 shadow-soft'
                                : 'text-slate-500 hover:text-slate-900 hover:bg-white/50'
                                }`}
                        >
                            <span
                                className={`material-symbols-outlined text-2xl ${active ? 'text-[#EB4C36]' : ''}`}
