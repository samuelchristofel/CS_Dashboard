...
export default function ResolutionTrendChart({ data, isLoading = false, period = "month" }: ResolutionTrendChartProps) {
  if (isLoading) {
    return (
      <div className="bg-white rounded-lg shadow-soft p-5 flex items-center justify-center h-[280px]">
  ...
  const getSubtitle = () => {
    switch (period) {
      case "today":
        return "Last 7 days • Target: < 24h";
      case "week":
        return "Last 7 days • Target: < 24h";
      case "month":
        return "Last 4 months • Target: < 24h";
      case "year":
        return "This year (12 months) • Target: < 24h";
      default:
        return "Last 6 months • Target: < 24h";
    }
  };
      ...
      return (
        <div className="bg-white rounded-lg shadow-lg border border-slate-200 p-3 text-sm">
          ...
          <p className="text-slate-600">
            Avg Time: <span className="font-bold">{value}h</span>
          </p>
          <p className="text-slate-600">
            Tickets: <span className="font-bold">{tickets}</span>
          </p>
  ...
  return (
    <div className="bg-white rounded-lg shadow-soft p-5 overflow-hidden h-full flex flex-col">
      <div className="mb-3">
      ...
      <div className="flex-1 w-full min-h-[180px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            ...
            <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "#64748b" }} tickFormatter={(value) => `${value}h`} />
            <Tooltip content={<CustomTooltip />} />
            <ReferenceLine y={24} stroke="#10b981" strokeDasharray="5 5" strokeWidth={2} label={{ value: "24h target", position: "right", fill: "#10b981", fontSize: 10 }} />
            <Area
              type="monotone"
              dataKey="avgHours"
              stroke="#3b82f6"
